...
func (s *TapScript) CreateRawTxP2TRMutiSig(
	prevHash *chainhash.Hash,
	prevIndex uint32,
	prevAmountSat int64,
	sendAddrStr string,
	...
	txIn.Witness = make([][]byte, len(witnessStack)+2+len(keys)) // signature + <witnessStack> + redeem_script + control_block
	for i := 0; i < len(keys) && i < threshold; i++ {
		witSig, err := txscript.RawTxInTapscriptSignature(
			originTx,
			sigHashes,
			txinIndex,
		...
		txIn.Witness[i] = witSig

	}

	for i := 0; i < len(witnessStack); i++ {
		txIn.Witness[len(keys)+i] = witnessStack[i]
	}
	txIn.Witness[len(txIn.Witness)-2] = s.Scripts[scriptIdx].Script
	txIn.Witness[len(txIn.Witness)-1] = cbBytes
